...
export function CityOverview({ city, info, onHighlightClick }: Props) {
  ...
  const showToggle = hasExtraSections || introNeedsClamp;

  if (!description && !hasExtraSections) return null;

  return (
    <Stack spacing={2.5} sx={{ mt: 2.5 }}>
      <SectionCard>
        <ClampedText
          body={description ?? `Learn more about ${city}.`}
          clampLines={INTRO_CLAMP_LINES}
          expanded={open || !introNeedsClamp}
        />
        {showToggle ? (
          <Box sx={{ mt: 1.25 }}>
            <ToggleButton
      ...
      {hasExtraSections ? (
        <Collapse in={open} timeout="auto" unmountOnExit>
          <Stack id={collapseId} spacing={2.5}>
            {info.history ? (
              <SectionCard>
                ...
                {hasPractical ? (
                  <SectionCard>
                    ...
                    <Stack spacing={1.25} sx={{ mt: 1.25 }}>
                      ...
                      {!hasClimateCard && info.climate ? (
                        <PracticalRow label="Climate" value={info.climate} />
                      ) : null}
                      {!hasClimateCard && info.bestTimeToVisit ? (
...
function SectionCard({ children }: { children: ReactNode }) {
  return (
    <Card elevation="flat">
      <Box sx={{ p: { xs: 2.5, sm: 2.5 } }}>{children}</Box>
    </Card>
  );
}
